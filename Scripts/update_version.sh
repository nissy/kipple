...
usage() {
    ...
    echo ""
    echo "Examples:"
    echo "  $0 1.1.1          # Update marketing version to 1.1.1"
    echo "  $0 1.1.1 42       # Update marketing version to 1.1.1 and build number to 42"
...
validate_version() {
    if [[ ! $1 =~ ^[0-9]+\.[0-9]+\.[0-9]+$ ]]; then
        echo -e "${RED}Error: Invalid version format. Expected: X.Y.Z (e.g., 1.1.1)${NC}"
        exit 1
    fi
}
