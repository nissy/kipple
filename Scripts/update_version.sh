...
usage() {
    ...
    echo ""
    echo "Examples:"
    echo "  $0 2.0.0          # Update marketing version to 2.0.0"
    echo "  $0 2.0.0 42       # Update marketing version to 2.0.0 and build number to 42"
...
validate_version() {
    if [[ ! $1 =~ ^[0-9]+\.[0-9]+\.[0-9]+$ ]]; then
        echo -e "${RED}Error: Invalid version format. Expected: X.Y.Z (e.g., 2.0.0)${NC}"
        exit 1
    fi
}
